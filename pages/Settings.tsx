...
const Settings: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            // 1. Listar todos los archivos en el prefijo del bucket
            ...
            // Filtrar para no incluir el propio playlist.json
            const mediaFiles = fileList.filter(file => file.name !== 'playlist.json');

            // 2. Intentar descargar el playlist.json existente
            ...
            // 3. Fusionar listas: usar el orden de la playlist existente y añadir nuevos archivos al final
            const existingPlaylistMap = new Map(existingPlaylist.map(item => [item.src.split('/').pop(), item]));
            const finalPlaylist: TVItem[] = [];
            const processedFiles = new Set<string>();

            // Añadir elementos en el orden de la playlist
            ...
            // Añadir archivos nuevos que no estaban en la playlist
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={fetchData} disabled={loading || saving} className="flex items-center justify-center rounded-lg bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 px-4 py-2 font-semibold text-gray-800 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50">
                     ...
                     <button onClick={handleSavePlaylist} disabled={loading || saving} className="flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700 disabled:bg-gray-500">
