import React, { useState } from 'react';
import type { Lead } from '../types';
import { db } from '../lib/supabaseClient'; // This is Firebase
import { collection, addDoc, serverTimestamp, query, where, getDocs, Timestamp } from 'firebase/firestore';
import { ORG_UUID, EVENT_CODE, EVENT_DATES } from '../lib/config';
import { LoaderCircle, UserPlus, PartyPopper, AlertTriangle } from 'lucide-react';
import { generateWelcomeMessage } from '../lib/geminiService';

interface LeadFormProps {
  onSuccess: (lead: Lead) => void;
  onReset: () => void;
  successLead: Lead | null;
}

const getCurrentEventDetails = (): { day: number; slot: 'AM' | 'PM' } => {
    const today = new Date();
    // Using UTC to be timezone-agnostic for event days
    const currentDay = today.getUTCDate(); 
    const currentHour = today.getUTCHours();
    const eventDays = EVENT_DATES.split(',').map(d => new Date(d.trim()).getUTCDate());
    
    const day = eventDays.includes(currentDay) ? currentDay : (eventDays[0] || new Date().getUTCDate());
    const slot = currentHour < 17 ? 'AM' : 'PM'; // Assuming event day ends at 5 PM UTC
    
    return { day, slot };
};

// FIX: Add export to make the component available for import.
export const LeadForm: React.FC<LeadFormProps> = ({ onSuccess, onReset, successLead }) => {
  const [formData, setFormData] = useState({
    name: '',
    company: '',
    role: 'Guia',
    whatsapp: '',
    email: '',
    interest: 'Ambos',
  });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [welcomeMessage, setWelcomeMessage] = useState<string>('');
  const [duplicateLead, setDuplicateLead] = useState<Lead | null>(null);

  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
  };
  
  const checkForDuplicates = async (): Promise<Lead | null> => {
    const trimmedWhatsapp = formData.whatsapp.trim();
    const trimmedEmail = formData.email.trim().toLowerCase();
    
    if (!trimmedWhatsapp && !trimmedEmail) {
      return null;
    }

    const queries = [];
    if (trimmedWhatsapp) {
        queries.push(where("whatsapp", "==", trimmedWhatsapp));
    }
    if (trimmedEmail) {
        queries.push(where("email", "==", trimmedEmail));
    }

    try {
        for (const condition of queries) {
            const q = query(collection(db, "leads"), where("event_code", "==", EVENT_CODE), condition);
            const querySnapshot = await getDocs(q);
            if (!querySnapshot.empty) {
                const doc = querySnapshot.docs[0];
                const data = doc.data();
                const createdAt = data.created_at instanceof Timestamp ? data.created_at.toDate().toISOString() : new Date().toISOString();
                
                // Construct a valid Lead object from the retrieved data
                const foundLead: Lead = {
                    id: doc.id,
                    created_at: createdAt,
                    org_id: data.org_id || ORG_UUID,
                    event_code: data.event_code || EVENT_CODE,
                    source: data.source || 'MANUAL',
                    day: data.day || 0,
                    slot: data.slot || 'AM',
                    name: data.name || 'N/A',
                    company: data.company || undefined,
                    role: data.role || undefined,
                    channel: data.channel || undefined,
                    whatsapp: data.whatsapp || undefined,
                    email: data.email || undefined,
                    interest: data.interest || undefined,
                };
                return foundLead;
            }
        }
    } catch (err) {
        console.error("Error checking for duplicates:", err);
    }
    return null;
  };

  const createLead = async () => {
    setLoading(true);
    setError(null);

    const { day, slot } = getCurrentEventDetails();

    // This new payload only includes the fields from the form,
    // avoiding sending `undefined` for optional fields like next_step.
    const leadPayload = {
      org_id: ORG_UUID,
      event_code: EVENT_CODE,
      source: 'MANUAL' as 'MANUAL',
      day,
      slot,
      name: formData.name.trim(),
      company: formData.company.trim() || undefined,
      role: formData.role as Lead['role'],
      channel: formData.role,
      whatsapp: formData.whatsapp.trim() || undefined,
      email: formData.email.trim().toLowerCase() || undefined,
      interest: formData.interest as Lead['interest'],
      tags: [],
    };

    try {
      const docRef = await addDoc(collection(db, 'leads'), {
        ...leadPayload,
        created_at: serverTimestamp(),
      });

      const newLead: Lead = {
        ...leadPayload,
        id: docRef.id,
        created_at: new Date().toISOString(),
      };
      
      onSuccess(newLead);
      generateWelcomeMessage(newLead).then(setWelcomeMessage);

    } catch (err: any) {
      console.error("Error adding lead:", err);
      setError("Could not save lead. Please check permissions.");
    } finally {
      setLoading(false);
      setDuplicateLead(null);
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name) {
      setError('Name is required.');
      return;
    }
    setLoading(true);
    const duplicate = await checkForDuplicates();
    if (duplicate) {
      setDuplicateLead(duplicate);
      setLoading(false);
    } else {
      await createLead();
    }
  };

  if (successLead) {
    return (
      <div className="text-center p-6 bg-green-50 dark:bg-green-900/20 border-2 border-dashed border-green-300 dark:border-green-700 rounded-xl animate-fade-in">
        <PartyPopper className="h-16 w-16 text-green-500 mx-auto mb-4" />
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Registration Successful!</h2>
        <p className="text-gray-600 dark:text-gray-300 mt-2">
          Registered <strong>{successLead.name}</strong> from <strong>{successLead.company || 'N/A'}</strong>.
        </p>
        {welcomeMessage && (
            <div className="mt-4 p-3 bg-green-100 dark:bg-green-900/50 rounded-lg text-sm text-green-800 dark:text-green-200">
                <p><strong>Message for TV screen:</strong> "{welcomeMessage}"</p>
            </div>
        )}
        <button
          onClick={() => {
            onReset();
            setFormData({ name: '', company: '', role: 'Guia', whatsapp: '', email: '', interest: 'Ambos' });
            setWelcomeMessage('');
          }}
          className="mt-6 w-full rounded-lg bg-blue-600 py-3 font-semibold text-white hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
        >
          <UserPlus size={18} />
          Register Another Attendee
        </button>
      </div>
    );
  }

  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Attendee Details</h2>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="name" className="label">Full Name *</label>
          <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} className="input" required />
        </div>
        <div>
          <label htmlFor="company" className="label">Company</label>
          <input type="text" id="company" name="company" value={formData.company} onChange={handleChange} className="input" />
        </div>
      </div>
      <div>
        <label htmlFor="role" className="label">Role</label>
        <select id="role" name="role" value={formData.role} onChange={handleChange} className="input">
          <option>Guia</option> <option>Agencia</option> <option>Hotel</option> <option>Mayorista</option> <option>Transportista</option> <option>Otro</option>
        </select>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="whatsapp" className="label">WhatsApp</label>
          <input type="tel" id="whatsapp" name="whatsapp" value={formData.whatsapp} onChange={handleChange} className="input" placeholder="+50612345678" />
        </div>
        <div>
          <label htmlFor="email" className="label">Email Address</label>
          <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} className="input" placeholder="name@company.com" />
        </div>
      </div>
       <div>
        <label htmlFor="interest" className="label">Main Interest</label>
        <select id="interest" name="interest" value={formData.interest} onChange={handleChange} className="input">
          <option>Ambos</option> <option>Tour</option> <option>Traslado</option>
        </select>
      </div>

      {duplicateLead ? (
          <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 text-yellow-800 dark:text-yellow-200">
              <div className="flex items-center">
                  <AlertTriangle className="h-6 w-6 mr-3 text-yellow-500"/>
                  <div>
                      <h3 className="font-bold">Possible Duplicate Found</h3>
                      <p className="text-sm">A lead with similar contact info already exists:</p>
                      <div className="mt-2 p-2 bg-yellow-100 dark:bg-yellow-800/30 rounded text-sm">
                          <strong>Name:</strong> {duplicateLead.name}<br/>
                          <strong>Company:</strong> {duplicateLead.company || 'N/A'}
                      </div>
                  </div>
              </div>
              <div className="mt-4 flex gap-3">
                  <button type="button" onClick={() => setDuplicateLead(null)} className="w-full rounded-lg bg-gray-500 py-2 font-semibold text-white hover:bg-gray-600">Cancel</button>
                  <button type="button" onClick={createLead} className="w-full rounded-lg bg-yellow-600 py-2 font-semibold text-white hover:bg-yellow-700">Create Anyway</button>
              </div>
          </div>
      ) : (
        <button type="submit" disabled={loading} className="w-full rounded-lg bg-blue-600 py-3 font-semibold text-white hover:bg-blue-700 disabled:bg-gray-500 dark:disabled:bg-gray-600 disabled:cursor-not-allowed transition-all flex items-center justify-center">
            {loading ? <><LoaderCircle className="animate-spin -ml-1 mr-3 h-5 w-5" /> Saving...</> : 'Register Attendee'}
        </button>
      )}

      {error && <p className="text-red-500 text-sm text-center">{error}</p>}
      <style>{`.label { @apply block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1; }`}</style>
    </form>
  );
};